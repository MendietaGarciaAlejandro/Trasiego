...
public class MovimientosEnBaseDeDatosTests(BaseDeDatosDePruebas baseDeDatos)
{
    ...
    public async Task Una_entrada_suma_al_saldo_y_una_salida_descuenta()
    {
        ...
        var saldo = await new RepositorioDeMovimientos(contexto).SaldoDe(articulo.Id, almacen.Id);

        Assert.Equal(Saldo.De(70), saldo);
    }
    ...
    public async Task El_saldo_a_fecha_no_cuenta_lo_que_vino_despues()
    {
        ...
        Assert.Equal(Saldo.De(40), await movimientos.SaldoDe(articulo.Id, almacen.Id, Escenario.Hoy.AddDays(-1)));
        Assert.Equal(Saldo.De(100), await movimientos.SaldoDe(articulo.Id, almacen.Id));
    }
    ...
    public async Task Un_movimiento_retroactivo_cambia_el_saldo_de_un_dia_ya_pasado()
    {
        ...
        Assert.Equal(Saldo.De(40), await movimientos.SaldoDe(articulo.Id, almacen.Id, elDiaDiez));
